defmodule TimelessMetrics.Application do
  ...
  def start(_type, _args) do
    Supervisor.start_link(configured_children(),
      strategy: :one_for_one,
      name: TimelessMetrics.AppSupervisor
    )
  end

  @doc false
  def configured_children(config \\ Application.get_all_env(:timeless_metrics))

  def configured_children(config) when is_list(config) do
    case Keyword.get(config, :owner, :embedded) do
      :external ->
        []
      ...
      owner ->
        raise ArgumentError,
              "invalid :timeless_metrics :owner #{inspect(owner)}; expected :embedded or :external"
    end
  end
end
