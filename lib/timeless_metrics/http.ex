defmodule TimelessMetrics.HTTP do
  ...
  get "/metrics" do
    ...
    lines = [
      "# HELP vm_memory_total Total BEAM memory usage in bytes.",
      "# TYPE vm_memory_total gauge",
      ~s(vm_memory_total{host="#{host}"} #{mem[:total]}),
      "# HELP vm_memory_processes BEAM process memory in bytes.",
      "# TYPE vm_memory_processes gauge",
      ~s(vm_memory_processes{host="#{host}"} #{mem[:processes]}),
      "# HELP vm_memory_ets BEAM ETS table memory in bytes.",
      "# TYPE vm_memory_ets gauge",
      ~s(vm_memory_ets{host="#{host}"} #{mem[:ets]}),
      "# HELP vm_memory_binary BEAM binary memory in bytes.",
      "# TYPE vm_memory_binary gauge",
      ~s(vm_memory_binary{host="#{host}"} #{mem[:binary]}),
      "# HELP vm_memory_atom BEAM atom memory in bytes.",
      "# TYPE vm_memory_atom gauge",
      ~s(vm_memory_atom{host="#{host}"} #{mem[:atom]}),
      "# HELP vm_memory_system BEAM system memory in bytes.",
      "# TYPE vm_memory_system gauge",
      ~s(vm_memory_system{host="#{host}"} #{mem[:system]}),
